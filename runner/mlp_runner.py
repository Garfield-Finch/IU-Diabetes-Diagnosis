"""
This was the root of base_runner.py
This is a legacy file and it is self-contained.
Further development in this codebase should NOT inherit from this file.
Further development in this codebase should inherit `base_runner.py`.
"""
...
class Net(nn.Module):
    def __init__(self, input_dim, ls_hidden_dim, n_classes=2, dropout_rate=0.5):
        super(Net, self).__init__()
        self.layers = nn.ModuleList()
        self.layers.append(nn.Linear(input_dim, ls_hidden_dim[0]))  # First layer

        # Adding hidden layers
        for i in range(len(ls_hidden_dim) - 1):
            self.layers.append(nn.Linear(ls_hidden_dim[i], ls_hidden_dim[i + 1]))

        self.out_layer = nn.Linear(ls_hidden_dim[-1], n_classes)  # Output layer
    ...
    def _initialize_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, nn.Linear):
                ...
                # Or Kaiming/He Initialization (better for ReLU)
                ...
                num_modules += 1
        print(f"   -- Initialize {num_modules} modules")
...
class ModelnData:
    ...
    # @staticmethod
    def cal_metrics(self, all_labels, all_predictions):
        precision = precision_score(all_labels, all_predictions, zero_division=0)
        recall = recall_score(all_labels, all_predictions, zero_division=0)
        accuracy = accuracy_score(all_labels, all_predictions)
        f1 = f1_score(all_labels, all_predictions, zero_division=0)
    ...
    def test(self, epoch):
        ...
        precision, recall, f1, accuracy = self.cal_metrics(all_labels, all_predictions)
        ...
        if self.is_print:
            ...
            # self.writer.add_scalar("Loss_epoch/test", loss_avg_epoch, epoch)
    ...
    def train(self, epoch):
        ...
        if self.is_print:
            ...
            # self.writer.add_scalar("Loss_epoch/train", loss_avg_epoch, epoch)
...
class BaseRunner:
    ...
    def run_and_organize_ans(self, mode="train", epoch=None, savemodel=False):
        ...
        include_PI_loss = False
        pi_loss = 0
        if "pi_loss" in ls_ans[0]:
            pi_loss = self.cal_weighted_average(ls_ans, item="pi_loss")
            include_PI_loss = True
        ...
        if self.wandbTracking:
            metrics = {"Precision" : precision,
                       ...
                       "Specificity" : specificity}
            if include_PI_loss:
                metrics.update({"PI Loss" : pi_loss})
            if mode == 'train':
                metrics = {"train" : metrics}
                wandb.log(metrics, commit=False)
            else:
                metrics.update({"Best_F1" : best_f1})
...
class ClusterModelRunner(BaseRunner):
    def __init__(
        self,
        exp_name=None,
        cluster_method=None,
        ...
        assert (LUPI_load_dir is None and LUPI_loss_mode is None) or (
            LUPI_load_dir is not None and LUPI_loss_mode is not None
        )
    ...
    def gen_exp_name(
        ...
        nm_code_clus = f"{nm_code_clus}-{cluster_idx}"

        # ## Code for classify_input
        if classify_input == "wo_lab":
            nm_code_input = "woLab"
        elif classify_input == "full":
            nm_code_input = "fullFeat"
        elif classify_input.startswith("cov>"):
    ...
    def setup_model_and_dataloader(
        ...
        """
        assert train_data_path is not None and test_data_path is not None
        assert cluster_method in ["cluster_v1", "cluster_v2"]

        if cluster_method in ["cluster_v1", "cluster_v2"]:
            print(f"Setting up model with cluster_method: {cluster_method}")
            # use data from cluster 0, 1, 2
            self.mlp = ModelnData(
                train_data_path=train_data_path,
                ...
                cluster_method=cluster_method,
                cluster_idx=cluster_idx,
                model_name=f"clus_{cluster_idx}",
                ls_hidden_dim=ls_hidden_dim,
                criterion=criterion,
                lr=lr,
                is_debug=self.is_debug,
    ...
    def run_and_organize_ans(self, mode="train", epoch=None):
        assert mode in ["train", "test"]
        ...
        ans = []
        if mode == "train":
            ans = self.mlp.train(epoch=epoch)
        elif mode == "test":
            ans = self.mlp.test(epoch=epoch)

        accuracy = ans["accuracy"]
        precision = ans["precision"]
        recall = ans["recall"]
        f1 = ans["f1"]
        loss = ans["loss"]

        # self.cal_overall_metrics(ls_ans)
        ...
        if mode == "train":
            ...
            if epoch >= 10:
                if "acc" in state:
                    self.mlp.save_model(
                        epoch=epoch, save_dir=self.save_dir, comment="acc"
                    )
                if "recall" in state:
                    self.mlp.save_model(
                        epoch=epoch, save_dir=self.save_dir, comment="recall"
                    )
                if "precision" in state:
                    self.mlp.save_model(
                        epoch=epoch, save_dir=self.save_dir, comment="precision"
                    )
                if "f1" in state:
                    self.mlp.save_model(
                        epoch=epoch, save_dir=self.save_dir, comment="f1"
                    )
        ...
        self.terminal_print(mode=mode, ans=ans, epoch=epoch)

    def load_model(
        self, load_dir=None, comment=None, pth_file_name=None, pth_file_path=None
    ):
        # assert (comment is not None and pth_file_name is not None) or (
        #     pth_file_path is None
        # )
        if pth_file_path is not None:
            assert comment is None and pth_file_name is None
        # else:
        #     # pth_file_name is None
        #     pth_file_name = f"{comment}_{self.model_name}.pth"
        else:
            # pth_file_name is None
            pth_file_path = os.path.join(load_dir, pth_file_name)
        ...
        # self.net.load_state_dict(torch.load(pth_file_path, map_location=self.device))

        state_dict = torch.load(pth_file_path, map_location=self.mlp.device)

        load_keys = set(state_dict.keys())
        model_keys = set(self.mlp.net.state_dict().keys())
        ...
        cur_state_dict = self.mlp.net.state_dict()
        ...
        self.mlp.net.load_state_dict(cur_state_dict, strict=True)
        print(f"===== Load model from: {pth_file_path} =====")
    ...
    def debug(self):
        ...
        #     )
        # tmp_mlp.append(mlp)
        self.mlp.load_model(load_dir=save_dir, pth_file_name="acc_clus_0.pth")

        print(f"After loading pretrained model parameters")
        self.test(epoch=1)
        self.test(epoch=1)
        # self.test(epoch=0)
        ...
        # if mode == "test":
        #     print(f"===== DEBUG =====")
        #     print(f"Test model saving and loading on the run")
        #     print(f"Epoch")
        #     self.mlp.save_model(epoch=epoch, save_dir=self.save_dir, comment="debug")

        #     self.mlp.load_model(
        #         pth_file_path=f"runs_clus/Jan11_12-17-18_clusV2-0_woLab_hiddenDim=[128, 64, 20]_lr=1e-06/bestAcc_clus_0.pth"
        #     )

        #     print(f"RUN the test for the loaded model")
        #     ans = self.mlp.test(epoch=epoch)
        #     self.terminal_print(mode=mode, ans=ans, epoch=epoch)

        #     self.mlp.load_model(
        #         load_dir=self.save_dir, pth_file_name="debug_clus_0.pth"
        #     )

        #     print(f"RUN rerun the test for the current model")
...
if __name__ == "__main__":
    # pass
    # runner = ClusterModelRunner(
    #     # exp_name=None,
    #     cluster_method="cluster_v2",
    #     cluster_idx=0,
    #     # cluster_idx=1,
    #     # cluster_idx=2,
    #     classify_input="wo_lab",
    #     # classify_input="wo_lab_diet",
    #     train_data_path="/home/zhi/pan_zhi/dataset/ada_train_victor_1:1.csv",
    #     test_data_path="/home/zhi/pan_zhi/dataset/ada_test_victor_1:1.csv",
    #     # train_data_path="/home/zhi/pan_zhi/dataset/ada_train_victor_2:1.csv",
    #     # test_data_path="/home/zhi/pan_zhi/dataset/ada_test_victor_2:1.csv",
    #     # hidden_dim=[128, 256, 512, 256, 20],
    #     # hidden_dim=[128, 128*3, 256, 20],
    #     # hidden_dim=[64, 128, 256, 20],
    #     hidden_dim=[64, 128, 20],
    #     # hidden_dim=[64, 64, 20],
    #     # hidden_dim=[64, 32, 20],
    #     # hidden_dim=[64, 20],
    #     # hidden_dim=[128, 64, 20],
    #     # lr=0.00002,
    #     lr=1e-6,
    #     num_epochs=5000,
    #     criterion="cross_entropy",
    #     exp_save_dir="runs_debug",
    # )
    # runner.run()

    # stat = ExpManager().stats(path_dir="runs_newft_LUPI", save_fp="stat_newftLUPI.csv")
    # ExpManager(is_debug=True).rm_error_logs("runs_newft")
    ...
    runner = BaseRunner(
        ...
        cluster_method=None,
        # classify_input="without_lab_encoded",
        # classify_input="no_lab_strict_encoded",
        # classify_input="priviledged_info_encoded",
        # classify_input="full",
        classify_input="wo_lab_enc",
        # classify_input="cov>0.1",
        # classify_input="cov>0.05",
        # classify_input="cov>0.02",
        # classify_input="cov>0.01",
        # classify_input="fullcov>0.1",
        # classify_input="fullcov>0.05",
        # classify_input="fullcov>0.02",
        # classify_input="fullcov>0.01",
        train_data_path="./victor_data/ada_test_victor_SMOTE_1:1_encoded_3_fold.csv",
        test_data_path="./victor_data/ada_train_victor_SMOTE_1:1_encoded_3_fold.csv",
        # train_data_path="/home/zhi/pan_zhi/dataset/ada_train_victor_2:1.csv",
        # test_data_path="/home/zhi/pan_zhi/dataset/ada_test_victor_2:1.csv",
        # hidden_dim=[128, 256, 512, 256, 20],
        # hidden_dim=[128, 128*3, 256, 20],
        # hidden_dim=[64, 128, 256, 20],
        hidden_dim=[64, 128, 20],
        # hidden_dim=[128, 256, 20],
        # hidden_dim=[128, 256, 256, 20],
        # hidden_dim=[100, 300, 200, 20],
        # lr=0.00002,
        lr=1e-6,
        # lr=5e-6,
        # criterion="cross_entropy",
        criterion="cross_entropy",
        # criterion="soft_label",
        exp_save_dir="runs_newft_M",
        LUPI_load_dir="runs_newftfull/Jan06_12-43-04_all_full_hiddenDim=[64, 128, 20]_lr=1e-06",
        # LUPI_loss_mode="diff_level",
        # LUPI_loss_mode="soft_label",
        is_debug=True,
    )
    runner.run()
    ...
    # runner = Runner("dataset/chunhao_file/post-process-noLUPI-sift.csv")
    # runner = RunnerArchive(
    #     "dataset/post-process-LUPI-victor.csv",
    #     is_debug=True,
    #     exp_name="balanced_victor_all",
    # )  # Zhening's dataset
    # runner.train()
