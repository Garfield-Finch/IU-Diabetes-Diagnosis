'''batching included on the PI version'''
...
#from runner.base_runner import ModelnData, BestRecord, cal_metrics, BaseRunner, DualOutput
...
#CONV layers tried: GCNConv, AGNNConv, GATv2Conv, GatedGraphConv
from torch_geometric.nn import SAGEConv, GCNConv, AGNNConv, GATv2Conv, GatedGraphConv
import numpy as np
...
class lupiEncoder(torch.nn.Module):
    def __init__(self, out_dim=76, batch_size=32):
...
class decoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        partial_edge_index, edge_mask = torch_geometric.utils.dropout_edge(edge_index)
        x = self.conv2(x, partial_edge_index)
        x = self.relu(x)
...
class ModelGNN(ModelnData):
    ...
    def gen_dataloader_train_LUPI(
        ...
        dataset = gnn_dataset.fastGNNLUPIDataset(data, data_LUPI, labels)
        #dataset = gnnLUPIDataset(data, data_LUPI, labels)
    ...
    def gen_dataloader(
        ...
        dataset = gnn_dataset.fastGNNDataset(data, labels)
        #dataset = gnnDataset(data, labels)
    ...
    def train(self, epoch):
        ...
        for i, (inputs, lupi) in enumerate(self.train_loader):
            ...
            if not self.pi_train and epoch < 50:
                ...
                lupi_loss2 = self.pi_criterion(r_i1, r_p1) * 0.1
...
class gnnLupiRunner(BaseRunner):
    def __init__(
        ...
        shutil.copy("gnnfork.py", os.path.join(self.save_dir, "gnnfork.py"))
        ...
        if cluster_method is None:
            ...
            self.ls_mlp = [
                ModelGNN(train_data_path=train_data_path, test_data_path=test_data_path, batch_size=batch_size, num_epochs=num_epochs,
                         pi_train=pi_train, pi_model_path=pi_model_path, criterion=criterion)
            ]
        else:
            original= pd.read_csv(train_data_path)
            self.ls_mlp = []
            for idx in original[cluster_method].unique():
                self.ls_mlp.append(
                    ModelGNN(train_data_path=train_data_path, test_data_path=test_data_path, batch_size=batch_size, num_epochs=num_epochs,
                         pi_train=pi_train, pi_model_path=pi_model_path, criterion=criterion,
                         cluster_method=cluster_method,
